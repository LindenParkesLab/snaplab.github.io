...
LLM-based AI models, including ChatGPT, Claude, Gemini, and Copilot, are very popular, and people are using them more and more to support their work. Below you will find an evolving list of our AI policies use in the lab. Note, this is a very active debate, and these policies will likely change quite a lot over time.

### Strict policies
1. Never upload any of the scientific data that we use to a cloud-based AI (e.g., Claude). 
 - **Doing so will likely breach the data use agreements (DUA) associated the dataset you are working with.** Nearly all open access datasets we use (e.g., from the Human Connectome Project) have their own DUA. This is a document that you sign acknowledging the conditions under which you are allowed to freely use that data. The vast majority of DUAs that exist in neuroscience today include language that prohibits you from unauthorized sharing of those data with a third party. AI companies like OpenAI (ChatGPT) and Anthropic (Claude) are, by definition, third parties. Thus, if you upload data to their servers, you are very likely in breach of the DUA. Importantly, data can be uploaded to AI chatbots very easily, even without you realizing. For example, if you prompt Claude to "Look in /my/data/folder on my laptop and describe its contents" you may have just uploaded that data to Anthropic's servers without being aware that you did. You must not do this. See this excellent [blog post](https://rgbayrak.github.io/blog/2026/claude-dua/) by Roza G. Bayrak for more information, as well as tips and tricks.
2. You take full responsibility for any outputs produced by AI.
 - **Just because Claude writes some of your code, does not make Anthropic responsible for its accuracy.** As scientists, we are responsible for what we put out into the world. If Claude provides you with a Python function to support your analysis that you don't understand, you need to take a step back and make sure you understand it before continuing. Using AI-generated code can lead to errors in our science, and it's our responsibility to minmize those errors. Errors and mistakes in science happen, and that's okay. AI models make mistakes, so do human scientists. But, if there are errors in our analysis, it's important we are the ones accountable for them, not the AI.
3. Disclose your use of AI.
 - **It's important that we're transparent about our use of AI,** so that we can participate---both as individuals and as a lab---in the discourse surrounding its use in academia. Disclosure includes both being transparent about your use of it in your work, discussing your use of it with Linden, and potentially acknowledging it in your publications.

### Less strict guidelines
There is a lot to discuss about how AI is used to enhance academic science, and people vary widely in what they believe constitutes (in)appropriate use. For example, is it okay to use AI to write your code but not your papers? Or is it okay to use it to write essentially anything, including code, papers, grants, etc? If it's not okay to use AI for writing, is it okay to use it for editing your work or to solicit feedback? These and many other questions are front of mind for lots of us; they certainly are for Linden!

However you use AI, we believe that intention matters a lot. Additionally, we believe that its important that you don't use AI to cheat yourself out of your own development as a scientist. There is value in learning a skill yourself. There is value in struggling (positively) with a hard problem. This is how we grow as scientists. With that in mind, we have a few guidelines to consider alongside the above policies.
1. **Regarding code:** we suggest using AI selectively to write specific functions and tools that fit into a broader analytic pipeline. That is, we think it's okay to ask AI to "build a function that takes two input variables, X and Y, and computes the [insert thing here]." We discourage broad unstructured prompts like "analyze this data for me, including creating all the figures and writing the report." Note, this second prompt may also cause you to violate the DUA (see above). Linden recognizes that there is *a lot* of space inbetween the above two examples (see note above about how this debate is actively evolving). Right now, no one knows where the line is, and there will probably never be a place to draw it that everyone agrees with. To help, here are some example use cases that we have found particularly impactful:
 - Code review and refactoring. For example, you may have a Python notebook that you know works and provides accurate outputs, but you might ask Claude to refactor it and streamline it. This will trigger Claude to generate a lot of code for you to check. This is okay, but again, the most important piece is that you verify it and its outputs.
 - Documenting code you've already written and verified. For example, you may pass a function (that you already know works) into Claude and ask it to provide a concise doc string, or check whether it has any edge cases that would cause silent errors.
 - [more examples coming soon]
2. **Regarding writing:** For Linden, writing is one of the most important skills you can develop as a scientist, and he holds it above coding in this regard. As such, Linden does not use AI to write, and generally discourages it. Again, intention and goals matter here. Linden believes its okay to give Claude a passage of your own text and ask it to provide feedback on clarity, flow, logic, etc. He doesn't think its appropriate to ask Claude to write whole sections of a paper or grant for you. Again, this is an open debate, and Linden is aware people disagree with him on this. As mentioned above, don't cheat yourself out of the hard work.
